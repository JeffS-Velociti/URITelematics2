({
	doInit : function(component, event, helper) {
        ...
        server.callServer(
            ...
            $A.getCallback(function(response) {
                ...
                component.set('v.backStatus','service detail');
    ...
    saveVehicleOnImageLoad : function(component, event, helper) {
        console.log('calling');
        helper.saveVehiclehelper(component, event, helper,false,false,true);
    },
    setFuels: function(component, event, helper){
        helper.setFuelsFromControllerType(component, event, helper);
    },
    barcodeScan : function(component, event, helper){
        console.log('telematics2');
        var dataAtt = event.currentTarget.dataset.att;
        var fieldName;
        if(dataAtt == 'RDSN'){
        ...
        myEvent.setParams({"fieldName" : fieldName, "mobileAppfieldName" : dataAtt});
        /*var param = {
            "browserFieldName" : "DSNSerial__c",
            "mobileAppfieldName" : dataAtt
        };
        myEvent.setParam('barcodeObject',param); */
        myEvent.fire();
        ...
        window.addEventListener("message", $A.getCallback(function(event) {
            ...
            if(eventData[2] == 'browser'){
                if(dataAtt == 'DSNSerial__c1'){
                    component.find("DeviceSerialNumber").set('v.value',eventData[0]);
                    vehicle1.DSNSerial__c = eventData[0];
                }else if(dataAtt == 'DSNSerial__c2'){
                    component.find("NewDeviceSerialNumber").set('v.value',eventData[0]);
                    vehicle1.DSNSerial__c = eventData[0];
                }else if(dataAtt == 'RDSN'){
                        ...
                        if(Array.isArray(required)){
                            for(var i=0;i<required.length;i++){
                                ...
                                if(fieldName == 'RDeviceSerialNumber'){
                                    required[i].set("v.value",eventData[0]);
                                    break;
                                }
                            }
                        }else{
                            ...
                            if(fieldName == 'RDeviceSerialNumber'){
                                required.set("v.value",eventData[0]);
                            }
                        }
                    }
    ...
    changeHandler : function(cmp, event, helper){
        ...
        helper.changeHandler(cmp, event, helper, name);
    }
